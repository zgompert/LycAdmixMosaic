...
void readData(string infile, datacont * data, int nh){
  ...
  // read line with sex data
  getline(file,line);//get rid of first extra hdr line, comes before sex data
  data->sex = gsl_vector_int_calloc(data->nind);
  getline(file, line);
  stream.str(line);
  stream.clear();
  for(j=0; j<data->nind; j++){
    stream >> element;
    gsl_vector_int_set(data->sex, j, atoi(element.c_str()));
  }
...
void expMax(datacont * data, gsl_matrix * af, double tol, int max, int i){
  ...
  double dif = 1;
  int sx = 0;
  double A = 0, B = 0; // two genotypes/alleles for haps
  double C = 0;
  double ngc = 0; // number of gene copies
  
  // initilize using simple point estimate
  for(j=0; j<data->nind; j++){
    sx = gsl_vector_int_get(data->sex, j);
    ngc+=sx;
    if(sx == 2){  
    ...
    if(sx == 1){
      A = gsl_matrix_get(data->gl[i], j, 0);
      B = gsl_matrix_get(data->gl[i], j, 2);
      C = A + B;
      A = A/C;
      B = B/C;
      p += B;
