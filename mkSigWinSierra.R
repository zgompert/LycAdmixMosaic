...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winCLHxTICxBHP/winout",ch,".tsv",sep="")
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winCPxYGxTIC/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
#[1] 0.31665706 0.11302942 0.03231711
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winMRxCPxYG/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
#[1] 0.33454768 0.20547475 0.07629971
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winCLHxYGxBHP/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
#[1] 0.09628636 0.45234740 0.04795728
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winTICxYGxBHP/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
#0.11601539 0.32624590 0.04780289
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winCPxYGxBHP/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
#0.19144952 0.22376185 0.04278196
...
scID<-vector("list",23)
for(ch in 1:23){
	inf<-paste("winEPxSHCxBHP/winout",ch,".tsv",sep="")
	dat<-read.table(inf,header=TRUE)
	L<-dim(dat)[1]
	scAB[[ch]]<-dat$A.B
	scAC[[ch]]<-dat$A.C
...
apply(mat[,3:5],2,mean)
## 0.14067904 0.19757930 0.04171554
...
## figures and tables
trs<-c("((CLH,TIC),BHP)","((CP,TIC),YG)","((MR,YG),CP)","((CLH,YG),BHP)","((TIC,YG),BHP)",
       "((CP,YG),BHP)","((EP,SHC),BHP)")
...
# & 1 & 2 & 3 & 4 & 5 & 6 \\
#  \hline
#((CLH,TIC),BHP) & 0.47 & 0.78 & 0.00 & 0.08 & 0.49 & 0.00 \\
#  ((CLH,YG),BHP) & 0.60 & 0.51 & 0.96 & 0.04 & 0.13 & 0.00 \\
#  ((TIC,YG),BHP) & 0.49 & 0.46 & 0.73 & 0.06 & 0.14 & 0.02 \\
#  ((CP,TIC),YG) & 0.45 & 0.69 & 0.00 & 0.09 & 0.02 & 0.00 \\
#  ((MR,YG),CP) & 0.61 & 0.76 & 0.00 & 0.11 & 0.36 & 0.00 \\
#  ((CP,YG),BHP) & 0.46 & 0.45 & 0.67 & 0.15 & 0.16 & 0.38 \\
#  ((EP,SHC),BHP) & 0.37 & 0.41 & 0.24 & 0.09 & 0.21 & 0.03 \\
#   \hline
#\end{tabular}
#\end{table}
...
pseq<-c(1,4,5,2,3,6,7)
pdf("fig_sigWinSierra.pdf",width=9,height=9)
par(mfrow=c(3,3))
...
pdf("fig_sigWin2Sierra.pdf",width=9,height=9)
par(mfrow=c(3,3))
par(mar=c(4,5,2.5,1.5))
cl<-1.4;ca<-1.1;ct<-1.05;cm<-1.3
k<-1
...
pdf("fig_dotWinSierra.pdf",width=7,height=7)
...
save(list=ls(),file="winsSierra.rdat")
