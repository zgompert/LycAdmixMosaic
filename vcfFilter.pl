...
## 295 inds, 2x
my $minCoverage = 590; # minimum number of sequences; DP
my $minAltRds = 10; # minimum number of sequences with the alternative allele; AC
my $notFixed = 1.0; # removes loci fixed for alt; AF
my $bqrs = 4; # abs Z score base quality bias; BQBZ
my $mqrs = 4; # abs Z mapping quality bias; MQBZ
my $rprs = 4; # abs Z read position biax; RPBZ
my $mq = 30; # minimum mapping quality; MQ
my $miss = 59; # maximum number of individuals with no data
##### this set is for GBS
my $d;
...
## bcftools call verions with z scores
		if(m/BQBZ=([0-9e\-\.]*)/){
			if (abs($1) > $bqrs){
				$flag = 0;
#				print "fail BQRS : ";
